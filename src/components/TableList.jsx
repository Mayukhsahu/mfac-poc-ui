...
const TableList = ({tableList}) => {
  const tableHeads = tableList.length !== 0 && Object.keys(tableList[0]).map(head => head.replaceAll("_", " "))
  return (
    <section className="p-3 sm:p-5">
      <div className="mx-auto max-w-screen-xl">
        <div className="bg-[#ecccbb] min-h-screen relative shadow-md sm:rounded-t-[50px] overflow-hidden">
          <div className="overflow-x-auto">
            {tableList.length !== 0 ? (
              <table className="w-full text-sm text-left text-gray-500 table-fixed">
                <thead className="text-xs text-[#ecccbb] uppercase bg-[#A06A50] w-full">
                  <tr className="">
                    {tableHeads.map((head) => (
                      <th
